...
class Edge {
  constructor(node1, node2, edge_weight) {
    this.node1 = node1;
    this.node2 = node2;
...
let bucket = []; // store all edges on the visited node, to find the best edge
...
function mstPrim(startNode) {
  ...
  while (bestEdge != null) {
    ...
    // for all visited nodes, check their edges
    allNodes.forEach((node) => {
      if (node.visited) {
        node.edges.forEach((edge) => {
          // the correct edge, not to check it again
...
function getBestEdge() {
  ...
  while (bestEdge == null && bucket.length != 0) {
    // find the best edge -> the "smallest edge_weight"
    bestEdge = bucket[0];
    let index = 0;
    bucket.forEach((edge, i) => {
      if (edge.edge_weight < bestEdge.edge_weight) {
        bestEdge = edge;
        index = i;
      }
    });
    // check in the "smallest edge_weight": whether node1 & node2 were visited
    if (bestEdge.node1.visited && bestEdge.node2.visited) {
      bucket.splice(index, 1); // remove the edge
      bestEdge = null; // in order to find the correct edge
    }
  }
